...
public class GenerateMotionProfile {
	...
	protected static MotionProfile generateFlippedInnerArcProfile(MotionProfileConstraints constraints,
			MotionProfileGoal goal_state, MotionState prev_state, double radius) {
		MotionProfile profile = generateInnerArcMotionProfile(constraints, goal_state.flipped(), prev_state.flipped(),
				radius);
		for (MotionSegment s : profile.segments()) {
			s.setStart(s.start().flipped());
			s.setEnd(s.end().flipped());
	...
	public static MotionProfile generateInnerArcMotionProfile(MotionProfileConstraints constraints,
			MotionProfileGoal goal_state, MotionState prev_state, double radius) {
		double delta_pos = goal_state.pos() - prev_state.pos();
		...
		if (delta_pos < 0.0 || (delta_pos == 0.0 && prev_state.vel() < 0.0)) {
			...
			// negative, we flip to solve, then
			// flip the solution.
			return generateFlippedInnerArcProfile(constraints, goal_state, prev_state, radius);
		}
		// Invariant from this point on: delta_pos >= 0.0
		// Clamp the start state to be valid.
		MotionState start_state = new MotionState(prev_state.t(), prev_state.pos(),
	...
	public static MotionProfile generateOuterArcMotionProfile(MotionProfile inner, double innerRadius,
			double outerRadius) {
